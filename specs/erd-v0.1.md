...
### 1.1 The connection carries the tenant and the principal, per transaction

Two settings decide what a query can reach, and both are set on the
connection by one layer (`app/infrastructure/database.py`) from the
authenticated session: `search_path` for the tenant schema, and the `app.*`
settings (`app.user_id`, `app.scope_kind`, `app.visible_user_ids`) that
row-level security policies read. Every table in every schema has row-level
security **enabled and forced**, with at least one policy; a test enumerates
the catalogue and fails on any table without both.

**The settings are transaction-local — `SET LOCAL`, or `set_config(…, true)` —
and never session-level.** This is a rule with evidence, not a preference.
Probed on 7 September 2026 through SQLAlchemy's asyncpg pool with one
connection: a principal set with `is_local => true` was gone when the next
request took the same connection (the setting read `''`, the policy returned
zero rows); a principal set at session level (`is_local => false`) rode the
connection into the next request and showed it another person's row. There
is no reset step to optimise away, because the transaction's end is the
reset; adding a session-level set anywhere reintroduces the leak.

Two consequences the same probe fixed: the application connects as a
non-superuser, non-owner role (`dcp_app`), because a superuser is exempt from
row-level security and FORCE does not change that; and every policy treats an
absent principal as no access, comparing through `coalesce(…, '')` rather
than `IS NOT NULL`, because a setting that has been set once in a session
reads back as `''` for the rest of it, not NULL.
`docs/phase3-item1-login-permissions.md` §5 has the probe results in full.
