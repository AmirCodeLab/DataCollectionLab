...
def main() -> int:
    ...
    print(
        f"\nRun `{REGENERATE}` and commit the result. An API change and its "
        "contract change belong in the same commit — that is the whole point "
        "of committing the contract.\n"
        "\nIf you did not touch the API, check whether fastapi or pydantic "
        "moved: the document is byte-for-byte what those versions emit, and a "
        "framework upgrade can reword a description. Regenerating is still the "
        "fix; the diff will show it was boilerplate."
    )
    return 1
